...
public class VerleihServiceImplTest
{
    ...
    @Test
    public void testKundeLeihtMedienAus()
    {
        List<Medium> testmedien = new ArrayList<>();
        testmedien.add(_abbey);
        testmedien.add(_shape);
        _verleihService.verleiheAn(_homer, testmedien, Datum.heute());
        assertTrue(_verleihService.sindAlleVerliehen(testmedien));
        //assertEquals(_verleihService.getAusgelieheneMedienFuer(_homer),testmedien);
    }

    @Test
    public void testMedienZuruecknehmen()
    {
        List<Medium> testmedien = new ArrayList<>();
        testmedien.add(_abbey);
        testmedien.add(_shape);
        _verleihService.verleiheAn(_homer, testmedien, Datum.heute());
        _verleihService.nimmZurueck(testmedien, Datum.heute());
        assertTrue(_verleihService.sindAlleNichtVerliehen(testmedien));
    ...
    private void setUpKunden()
    {
        ...
        List<Kunde> testkunden = new ArrayList<>();
    ...
    private void setUpMedien()
    {
        ...
        List<Medium> _medien = new ArrayList<>();
        _medien.add(_abbey);
        _medien.add(_bad);
        _medien.add(_shape);
